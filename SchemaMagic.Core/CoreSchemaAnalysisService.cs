...
public partial class CoreSchemaAnalysisService
{
	...
	public static SchemaAnalysisResult AnalyzeDbContextContent(string dbContextContent)
	{
		try
		{
			var entities = AnalyzeEntitiesFromContent(dbContextContent);
	...
	private static Dictionary<string, EntityInfo> AnalyzeEntitiesFromContent(string sourceCode)
	{
	...
	{
		var match = DbSetRegex().Match(dbSetType);
		return match.Success ? match.Groups[1].Value : null;
	}

	[GeneratedRegex(@"DbSet<(.+?)>")]
